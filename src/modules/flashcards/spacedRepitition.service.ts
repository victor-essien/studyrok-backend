...
/**
   ...
   */
  calculateStreak(lastReviewDate: Date | null, currentDate: Date = new Date()): boolean {
   ...
   */
  getOptimalReviewTime(studyHistory: Array<{ hour: number; accuracy: number }>) {
    if (studyHistory.length === 0) {
      return 9; // Default to 9 AM
    }

    // Find hour with best accuracy
    const hourStats = studyHistory.reduce((acc, session) => {
   ...
   */
  predictRetention(interval: number, easeFactor: number, daysSinceReview: number): number {
